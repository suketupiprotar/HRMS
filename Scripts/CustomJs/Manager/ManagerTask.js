...
$(document).ready(function () {
    console.log("In DirectorTask Js")
    bindDatatable();
});
    ...
    table = $("#ManagerTask")
        .DataTable({
            ...
            "columns": [
                ...
                {
                    "data": "TaskDate", "autoWidth": true, "searchable": true,
                ...
                },
                {
                    ...
                    "autoWidth": true,
                    "render": function (data, type, row) {
                        var disabled = row.Status.trim().toLowerCase() === "approve" || row.Status.trim().toLowerCase() === "reject" ? "disabled" : "";
                        return '<button class="btn btn-success gap-3 managerTaskCreateOrUpdate" data-id="' + row.TaskId + '" ' + disabled + '>Edit</button>' +
...
$(document).on("click", ".managerTaskCreateOrUpdate", function () {
    var id = $(this).data("id");
    $.ajax({
        type: "GET",
        url: "/Manager/TaskCreate/" + id,
...
$(document).on("submit", "#taskForm", function (e) {
    e.preventDefault();
    console.log('In Submit Ajax');
    var taskData = new FormData(this);
    console.log(taskData);
    $.ajax({
        type: "POST",
        url: '/Manager/TaskCreate',
        data: taskData,
        processData: false,
        contentType: false,
        success: function (response) {
...
$(document).on("click", ".deleteButton", function () {
    ...
    $.ajax({
        type: "GET",
        url: "/Manager/DirectorTaskDelete/" + id,
        success: function (response) {
            if (response.success) {
