...
namespace HRMSWithTheme.Controllers
{
    ...
    public class DirectorController : Controller
    {
        ...
        public JsonResult GetTasksForEmployee(int empid)
        {
            try
           {
                // Fetch tasks for the specified employee
                //var tasks = entity.TaskMasters.Where(t => t.EmployeeId == empid);

                var tasks = entity.TaskMasters.ToList().Where(e => e.EmployeeId == empid).Select(task => new
                {
                    TaskId = task.TaskId,
                    TaskName = task.TaskName,
                    EmployeeId = task.EmployeeId,
